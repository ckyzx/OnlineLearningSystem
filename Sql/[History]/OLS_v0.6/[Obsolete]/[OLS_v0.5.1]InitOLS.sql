...
/* Created on:     2015/12/26 16:22:43                          */
/*==============================================================*/
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('ExaminationPapers') and o.name = 'FK_EXAMINAT_EP_ET_EXAMINAT')
alter table ExaminationPapers
   drop constraint FK_EXAMINAT_EP_ET_EXAMINAT
go

if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('ExaminationTaskAttendees') and o.name = 'FK_EXAMINAT_ETA_ET_EXAMINAT')
alter table ExaminationTaskAttendees
   drop constraint FK_EXAMINAT_ETA_ET_EXAMINAT
go
